...
try:
    ...
    def 地标视角校准():
        ...
        for j in range(1, 21):
            ...
            if 地标坐标:
                地标视角校准_遮挡 = 1
                x1, y1, x2, y2 = 地标坐标
                地标视角校准_历史坐标 = int((x1 + x2) / 2)
                偏差值 = 地标视角校准_历史坐标 - 屏幕中心x
                if 偏差值 > x相对坐标(130):
                    随机小幅度划屏((x相对坐标(1278), y相对坐标(692)), 'right', x相对坐标(45))
                    log.debug("➡️ 标识符偏右，控制摇杆往右前方推，或者向右微调视角")
                    time.sleep(0.5)
                    # 你的摇杆控制逻辑...
                elif 偏差值 < -x相对坐标(130):
                    随机小幅度划屏((x相对坐标(1278), y相对坐标(692)), 'left', x相对坐标(45))
                ...
                if 地标视角校准_历史坐标:
                    if x相对坐标(981) < 地标视角校准_历史坐标 < x相对坐标(1494):
                        log.debug('地标在正前方消失，默认视角已对准，返回True')
                        地标视角校准_遮挡 = 0
                        地标视角校准_历史坐标 = None
                        return True
                    else:
                        log.debug('地标在非正前方消失，未对准，滑动屏幕中')
                        随机小幅度划屏((x相对坐标(1278), y相对坐标(692)), 'left', x相对坐标(130))
                        time.sleep(0.5)
                地标视角校准_历史坐标 = None
                地标视角校准_遮挡 = 0
            else:
                log.debug('地标不存在，滑动屏幕中')
                随机小幅度划屏((x相对坐标(1278), y相对坐标(692)), 'left', x相对坐标(130))
                time.sleep(0.5)
            log.debug(f'地标视角校准_历史坐标:【{地标视角校准_历史坐标}】')
    ...
    def 血条检测(roi=config.血条roi, pixel_threshold=40):
        ...
        # 7. 打印详细调试信息，方便你调参
        log.debug(f"📊 【血条检测】 目标区域符合条件的像素数: {matching_pixels} (目标阈值: {pixel_threshold})")
    ...
    def 寻敌状态检测(状态参数):
        ...
        # 2. 条件一：如果第一个值就为 False（没血条），立刻返回 True
        if not 是否有血条:
            # 💡 注意：没怪的时候，把记忆重置为 None，防止下一波怪刷出来时，跟上一波怪的死前残余像素做对比
            log.debug("⚠️ 【寻敌状态检测】 当前没有血条，默认视为寻敌状态，重置上次像素值")
            _上次的像素值 = None
            return True
    ...
    def 连续性检测(检测数据,连续次数):
        ...
        if 连续出现次数 >= 连续次数:
            连续出现次数 = 1
            return 上一次结果
        log.debug('【连续性检测】连续性检测未达到次数，返回空')
    ...
    def 终点标识符是否存在():
        """
        检测当前画面是否存在终点标识符，如果存在，返回True，否则
        返回False
        :return:
        """
    ...
    def 终点标识符检测():
        ...
        if 连续性过滤:
            ...
            if 偏差值 > x相对坐标(150):
                随机小幅度划屏((x相对坐标(1278),y相对坐标(692)),'right',x相对坐标(55))
                log.info("➡️ 标识符偏右，控制摇杆往右前方推，或者向右微调视角")
                time.sleep(0.5)
                # 你的摇杆控制逻辑...
            elif 偏差值 < -x相对坐标(150):
                随机小幅度划屏((x相对坐标(1278), y相对坐标(692)),'left',x相对坐标(55))
