...
def 图像是否存在(img_name, threshold=0.7, gray_mode=False):
    """
    检测图像是否存在
...
def 图像随机位置点击(img_name, threshold=0.7, padding=0):
    """
    匹配模板并在匹配区域内返回一个随机坐标
    ...
    print(f'图像随机位置点击函数置信度：{max_val}')
    if max_val >= threshold:
        # 获取匹配区域的左上角 (x, y)
        x1, y1 = max_loc
        print(f'匹配区域左上角{(x1,y1)}')
...
def 怪物名检测(roi=config.怪物名roi, pixel_threshold=相对面积(40)):
    """
...
def 血条检测(roi=config.血条roi, pixel_threshold=相对面积(40)):
    """
    检测屏幕指定 ROI 区域内符合 HSV 范围的像素数量
    :param img: 传入的当前最新 OpenCV 格式截图 (BGR)
    :param roi: 检查区域 [x1, y1, x2, y2]
...
def 获取最右侧未通关关卡(img, boxes, threshold=0.85):
    ...
    for box in boxes:
        ...
        for state, data in LOADED_TEMPLATES.items():
            ...
            if cv2.countNonZero(mask) > 相对面积(100):
                masked_roi = cv2.bitwise_and(roi_gray, roi_gray, mask=mask)
                res = cv2.matchTemplate(masked_roi, data['gray'], cv2.TM_CCOEFF_NORMED)
                _, max_val, _, _ = cv2.minMaxLoc(res)
...
def 战斗标识检测(image, roi=(x相对坐标(2065), y相对坐标(1219), x相对坐标(68), y相对坐标(81))):
    ...
    return 1 if pixel_count > 相对面积(500) else None
...
def 检查区域是否命中章节标签黑名单(blacklist_dict=config.章节标签黑名单, roi=config.章节标签roi, threshold=0.7):
    """
...
def 是否命中章节黑名单(blacklist_dict=config.章节黑名单, roi=config.章节roi, threshold=0.7):
    """
    检测屏幕上指定区域内是否存在字典里的模板图片
    :param blacklist_dict: 你的黑名单字典（包含标签名和对应的图片绝对路径）
    :param roi: 指定区域的坐标，格式为 [x1, y1, x2, y2]。如果为 None，则全屏检测
...
def 消球检测(img):
    ...
    for index, (pt_x, pt_y) in enumerate(dict_a):
        # 截取采样区域 (±65像素)
        sample = hsv_img[pt_y - y相对坐标(50):pt_y + y相对坐标(50), pt_x - x相对坐标(50):pt_x + x相对坐标(50)]
        ...
        for name, bounds in BALL_COLORS.items():
            ...
            if cv2.countNonZero(mask) > 相对面积(1500):
                return name
...
def 必杀检测(image):
    ...
    roi = (x相对坐标(57), y相对坐标(123), x相对坐标(423), y相对坐标(8))
    # 1. 转换颜色空间
    # image=截图()
